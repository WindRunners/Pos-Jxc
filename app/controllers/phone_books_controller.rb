class PhoneBooksController < ApplicationController
  ...
  def index
    @phone_books = PhoneBook.order(:created_at => "DESC").page(params[:page]).per(10)
  end
  ...
  def create
    ...
    @phone_book.creator=current_user
    p phone_book_params

    respond_to do |format|
      if @phone_book.save
        format.js { render_js phone_books_path,'Phone book was successfully created.' }
  ...
  def upload
    ...
    asset = params[:phone_book][:photo] #上传的图片文件

    unless asset.blank?
      @phone_book.photo=asset
    end
    if @phone_book.save
      asset=nil
      respond_to do |format|
  ...
  def update
    respond_to do |format|
      if @phone_book.update(phone_book_params)
        format.js { render_js phone_books_path,'Phone book was successfully updated.' }
  ...
  def destroy
    @phone_book.destroy
    respond_to do |format|
      format.js { render_js phone_books_path,'Phone book was successfully destroyed.' }
  ...
  def phone_book_params
    params.require(:phone_book).permit(:telephone, :name, :url, :city, :nature, :address)
  end
end
