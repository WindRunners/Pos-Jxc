...
var from_submit = function () {
    ...
    $.ajax({
        ...
        type: "post",
        url: "/chateaus/"+chateau_id+"/relate_wine",
        data: {wine_id: wine_id._id,chateau_id: chateau_id},
        dataType: "json",
        ...
        success: function (data) {
            ...
            $('#resultTable').prepend("<tr><td>" + data.name + "</td><td>" + data.category + "</td><td>" + data.price + "</td>" +
                ...
                "<td><a  class='btn btn-minier btn-danger' href='/chateaus/" + chateau_id + "/resolve_wine?wine_id=" + wine_id._id + "'rel='nofollow' data-confirm='Are you sure?'>解除关联</a></td></tr>")
            alert("关联成功！");
        }, error: function (data) {
            $('#exampleModal').modal('hide');
            alert("关联失败！");
