class JxcSellReturnsBill
  ...
  def audit(current_user)
    ...
    if self.bill_status == '0'
      #单据商品详情
      billDetailsArray = JxcBillDetail.includes(:product).where(:sell_returns_bill_id => self.id)
      #仓库
      ...
      if billDetailsArray.present?
        billDetailsArray.each do |billDetail|
          ...
          storageJournal.jxc_sell_returns_bill = self  #库存变更依据的 单据
          storageJournal.bill_no = self.bill_no #单据编号
          storageJournal.bill_type = 'sell_returns'
